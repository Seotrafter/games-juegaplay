...
*/
    ...
    class Cart {
        ...
        getProductData(productCard) {
            ...
            if (!productId || !productImage || !productTitle || !productPrice) throw new Error("Missing product data");
            return {
                id: productId,
                image: productImage,
                title: productTitle,
        ...
        createCartItemHTML(item) {
            if (!item || !item.id || !item.image || !item.title || !item.price) return "";
    ...
    function initReviews() {
        ...
        const reviewsInStorage = JSON.parse(localStorage.getItem("helpfulReviews") || "{}");
        helpfulButtons.forEach((button => {
            const reviewCard = button.closest(".review-card");
            if (!reviewCard) return;
            const reviewId = reviewCard.dataset.reviewId || Math.random().toString(36).substr(2, 9);
            if (!reviewCard.dataset.reviewId) reviewCard.dataset.reviewId = reviewId;
            const countSpan = button.querySelector("span");
            if (!countSpan) return;
            let currentCount = parseInt(countSpan.textContent.match(/\d+/)[0]);
            if (reviewsInStorage[reviewId]) {
                currentCount = reviewsInStorage[reviewId].count;
                if (reviewsInStorage[reviewId].voted) button.classList.add("review-card__helpful--active");
            } else {
                reviewsInStorage[reviewId] = {
                    count: currentCount,
                    voted: false
                };
                localStorage.setItem("helpfulReviews", JSON.stringify(reviewsInStorage));
            }
            updateHelpfulCount(button, currentCount);
            button.addEventListener("click", (() => {
                if (!reviewsInStorage[reviewId].voted) {
                    const newCount = currentCount + 1;
                    reviewsInStorage[reviewId].count = newCount;
                    reviewsInStorage[reviewId].voted = true;
                    localStorage.setItem("helpfulReviews", JSON.stringify(reviewsInStorage));
                    button.classList.add("review-card__helpful--active");
                    gsapWithCSS.to(button, {
                        ...
                        onComplete: () => {
                            ...
                            updateHelpfulCount(button, newCount);
                    ...
                    const newCount = currentCount - 1;
                    reviewsInStorage[reviewId].count = newCount;
                    reviewsInStorage[reviewId].voted = false;
                    localStorage.setItem("helpfulReviews", JSON.stringify(reviewsInStorage));
                    gsapWithCSS.to(button, {
                        scale: .9,
                        duration: .1,
                        onComplete: () => {
                            ...
                            button.classList.remove("review-card__helpful--active");
                            updateHelpfulCount(button, newCount);
                            currentCount = newCount;
                        }
                    });
    ...
    function updateHelpfulCount(button, count) {
        ...
        if (span) span.textContent = `Útil (${count})`;
    }
    function createFloatingHearts(button) {
        ...
        for (let i = 0; i < hearts; i++) {
            ...
            heart.className = "fas fa-heart review-card__floating-heart";
            document.body.appendChild(heart);
            const heartRect = heart.getBoundingClientRect();
            const startX = buttonRect.left + buttonRect.width / 2 - heartRect.width / 2;
            const startY = buttonRect.top + buttonRect.height / 2 - heartRect.height / 2;
            const randomX = -20 + Math.random() * 40;
            const randomY = -30 - Math.random() * 20;
            const randomRotate = -30 + Math.random() * 60;
            gsapWithCSS.fromTo(heart, {
                x: startX,
                y: startY,
                scale: .5,
                opacity: 1
    ...
    function initializeNewsletterForm() {
        ...
        const showError = (field, message) => {
            const input = field instanceof HTMLElement ? field : newsletterForm.querySelector(`#newsletter-${field}`);
            const errorElement = newsletterForm.querySelector(`[data-error="${field}"]`);
        ...
        newsletterForm.querySelectorAll("input").forEach((input => {
            input.addEventListener("input", (() => {
                if (input.type === "checkbox") hideError("consent"); else hideError(input.id.replace("newsletter-", ""));
            }));
        }));
        newsletterForm.addEventListener("submit", (e => {
            e.preventDefault();
            let isValid = true;
            newsletterForm.querySelectorAll(".newsletter__form-error").forEach((error => {
            ...
            if (!consentCheckbox.checked) {
                showError("consent", "Por favor, acepte los términos y condiciones");
                isValid = false;
            }
            if (isValid) window.location.href = "thank-you.html";
        }));
    }
